...
TOKEN=$(sed -n 's|https://Disable-OP:\([^@]*\)@github.com|\1|p' "$HOME/.git-credentials")
# 6-Z225: GITHUB_TOKEN fallback (same as dispatch_by_name.sh) — the dev
# sandbox provides GITHUB_TOKEN via the private agent environment (exported as
# GITHUB_TOKEN by the caller); ~/.git-credentials does not always exist.
[ -n "$TOKEN" ] || TOKEN="${GITHUB_TOKEN:-}"
[ -n "$TOKEN" ] || { echo "no token in .git-credentials or GITHUB_TOKEN"; exit 1; }
