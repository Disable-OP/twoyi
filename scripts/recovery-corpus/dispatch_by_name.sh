...
# Uses the git-credentials store or GITHUB_TOKEN for the token. Manifest must be in corpus/manifest.yaml.
set -u
...
TOKEN=$(sed -n 's|https://Disable-OP:\([^@]*\)@github.com|\1|p' "$HOME/.git-credentials" 2>/dev/null)
if [ -z "$TOKEN" ]; then
  # Try the token from the environment (used by the dev sandbox).
  TOKEN="${GITHUB_TOKEN:-}"
fi
