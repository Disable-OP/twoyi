...
"emulator_renderer -windowid %d -port %d -x %d -y %d -width %d -height %d"
,
...
//      a request
        ERR
(
"%s not implemented for separate renderer process !!!\n"
,
            __FUNCTION__
);
 ...
 */
#define
  DEFAULT_STREAM_MODE  STREAM_MODE_TCP
int
 gRendererStreamMode 
=
 DEFAULT_STREAM_MODE
 ...
 
{
        ERR
(
"createRenderThread failed to create stream\n"
);
...
for (let lineNumEl of document.querySelectorAll('td.u-lineNum')) {lineNumEl.onclick = () => {window.location.hash = `#${lineNumEl.getAttribute('data-line-number')}`;};}
