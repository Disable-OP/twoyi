...
| Thing | Path |
|---|---|
| Dev branch | `origin/improvements/initial-cleanup` (43 commits ahead of base `25ef89c`) |
| Local branch | `main` — 47 ahead of `origin/main`, branch-hygiene debt |
...
| CI workflows | `.github/workflows/build.yml`, `kr64-tests.yml` |
...
git checkout improvements/initial-cleanup && git pull   # 43 commits of overnight work
git status                                              # 57 untracked .md — decide their fate
...
cd app/rs && cargo test -p kr64 --no-fail-fast          # 38 host-runnable tests
...
- **`main`** is 47 ahead of `origin/main` and isn't the dev branch. Reconcile before tagging `v0.1.0`.
...
- **Bus factor = 1.** 43 commits, 0 human-reviewed PRs. First PR to review: #1.
