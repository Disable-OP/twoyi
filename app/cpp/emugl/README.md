...
AOSP tree are deliberately **not** included (they're platform-specific
X11 / Win32 / Carbon code that doesn't apply to Android).
...
| `libOpenglRender/render_api.cpp` | `static RenderServer *s_renderThread` → `RenderServer *s_renderThread` | let `twoyi/twoyi_api.cpp` reference it |
...
| File | Purpose |
|---|---|
| `twoyi/twoyi_api.cpp` | implements the 6 twoyi-flavored C-ABI entry points (`startOpenGLRenderer`, `setNativeWindow`, `resetSubWindow`, `removeSubWindow`, + the `dl*_ex` legacy wrappers) by composing the AOSP `FrameBuffer` / `RenderServer` API |
| `twoyi/NativeAndroidSubWindow.cpp` | replaces AOSP's X11-based `NativeLinuxSubWindow.cpp`; on Android the ANativeWindow IS the EGLNativeWindow, so `createSubWindow` just hands it back |
| `compat/` | shim layer for the Android platform-private headers that AOSP emugl uses (`cutils/{threads,atomic,log,sockets}.h`, `utils/{threads,Errors,Vector,List,String8,KeyedVector,RefBase}.h`) — implemented on top of pthreads / std::vector / std::map / liblog |
...
file). The twoyi-specific additions (`twoyi/`, `compat/`) are Mozilla
Public License 2.0.
