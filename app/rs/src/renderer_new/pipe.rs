...
impl PipeConnection {
    ...
    pub fn new(service_name: &str) -> io::Result<Self> {
        info!("Opening QEMU pipe connection to: {}", service_name);
        ...
        info!("Successfully connected to QEMU pipe: {}", service_name);
        Ok(connection)
    }
    ...
    fn connect(&mut self) -> io::Result<()> {
        ...
        self.file.write_all(service_bytes)?;
        self.file.flush()?;
        
        debug!("Sent service name: {}", self.service_name);
        Ok(())
    }
