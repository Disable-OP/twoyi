...
pub fn write_vendor_default_prop(
    ...
    // 6-Z305t-6: if the guest ROM SHIPS its own /vendor/default.prop
    // (the Android 11 SDK image ships ro.vndk.version=30,
    // ro.bionic.arch=arm64, ro.bionic.cpu_variant, dalvik.vm.isa.*,
    // ro.logd.size.stats, …), APPEND the twoyi container block instead
    // of overwriting. The old fs::write (O_TRUNC) ERASED the ROM's
    // vendor props: linkerconfig then CHECK-failed `context.cc:115]
    // Check failed: !"undefined var" VENDOR_VNDK_VERSION is not
    // defined` (it reads ro.vndk.version), produced NO output, and the
    // early-init copy left an empty /linkerconfig/default/ld.config.txt
    // → no /apex linker namespaces → lmkd CANNOT LINK
    // libstatssocket.so ×5 → critical ×4 → InitFatalReboot (ladder
    // 34113686284/#56). Preserving the shipped props is honest
    // flattened-APEX/system-as-root semantics; the container block is
    // additive exactly like the ro.apex.updatable append (798).
    //
    // Legacy note: a /vendor/default.prop that a PREVIOUS build already
    // truncated to the stub cannot be resurrected (content lost before
    // this fix) — the block-append is still correct from now on. A
    // file that already carries the block (previous boot, persistent
    // rootfs) is left untouched (idempotent via the header marker).
    match fs::read_to_string(&default_prop) {
        Ok(existing) if existing.contains("proc_emu::write_vendor_default_prop") => {
        ...
        Ok(_existing) => {
            ...
            let mut f = fs::OpenOptions::new()
                ...
                .map_err(|e| {
                    std::io::Error::new(
                        e.kind(),
                        format!("append to existing {} failed: {}", default_prop, e),
                    )
                })?;
            f.write_all(content.as_bytes())?;
        }
        Err(_) => {
            fs::write(&default_prop, &content.trim_start_matches('\n'))?;
        }
    }
...
mod tests {
    ...
    /// 6-Z305t-6 regression: when the guest ROM SHIPS its own
    /// /vendor/default.prop, `write_vendor_default_prop` must APPEND the
    /// container block and PRESERVE the shipped props. The old
    /// fs::write(O_TRUNC) erased ro.vndk.version (and ro.bionic.*,
    /// dalvik.vm.isa.*, …), linkerconfig CHECK-failed
    /// "VENDOR_VNDK_VERSION is not defined" and emitted NO config →
    /// empty default/ld.config.txt → lmkd CANNOT LINK libstatssocket.so
    /// → InitFatalReboot (ladder 34113686284/#56). The appended block
    /// must also be IDEMPOTENT (a second call on the same rootfs must
    /// not duplicate it).
    #[test]
    fn test_vendor_default_prop_preserves_rom_props_and_appends() {
        let rootfs = tmpdir();
        let dir = format!("{}/vendor", rootfs);
        std::fs::create_dir_all(&dir).unwrap();
        let prop_path = format!("{}/default.prop", dir);
        // A real Android 11 image's vendor/default.prop (abridged to the
        // props whose loss actually broke the boot).
        let rom_props = "# ADDITIONAL VENDOR DEFAULT PROPERTIES\n\
                         ro.vndk.version=30\n\
                         ro.zygote=zygote64\n\
                         ro.bionic.arch=arm64\n\
                         dalvik.vm.isa.arm64.variant=generic\n";
        std::fs::write(&prop_path, rom_props).unwrap();

        write_vendor_default_prop(&rootfs, 320, "en", "US", "America/New_York")
            .expect("write_vendor_default_prop (append)");

        let content = std::fs::read_to_string(&prop_path).unwrap();
        // Shipped props survive verbatim:
        assert!(
            content.contains("ro.vndk.version=30"),
            "ROM prop ro.vndk.version erased: {}",
            content
        );
        assert!(
            content.contains("ro.bionic.arch=arm64"),
            "ROM prop ro.bionic.arch erased: {}",
            content
        );
        assert!(
            content.contains("dalvik.vm.isa.arm64.variant=generic"),
            "ROM prop dalvik.vm.isa.arm64.variant erased: {}",
            content
        );
        // Container block appended:
        assert!(
            content.contains("ro.hardware=goldfish"),
            "container block missing: {}",
            content
        );
        assert!(
            content.contains("ro.sf.lcd_density=320"),
            "container block missing: {}",
            content
        );
        // Idempotent second call:
        write_vendor_default_prop(&rootfs, 320, "en", "US", "America/New_York")
            .expect("write_vendor_default_prop (idempotent)");
        let content2 = std::fs::read_to_string(&prop_path).unwrap();
        assert_eq!(
            content2.matches("ro.hardware=goldfish").count(),
            1,
            "container block duplicated on second call"
        );
